...
#ifndef CPROVER_IMPARA_INTERVALS_H
#define CPROVER_IMPARA_INTERVALS_H
...
public:
  ...
  explicit intervalt(const T &value):
  ...
  bool join(const intervalt<T> &other);

  // Intersection or conjunction
  bool meet(const intervalt<T> &other);
};

// return 'true' if there is change
template<typename T>
bool intervalt<T>::make_le_than(const T &other)
{
  if(!upper_set)
  {
    set_upper(other);
...
// return 'true' if there is change
template<typename T>
bool intervalt<T>::make_ge_than(const T &other)
{
  if(!lower_set)
  {
    set_lower(other);
...
// return 'true' if there is change
template<typename T>
bool intervalt<T>::join(const intervalt<T> &other)
{
  bool result=false;
...
template<typename T>
bool intervalt<T>::meet(const intervalt<T> &other)
{
  bool result=false;
...
typedef intervalt<mp_integer> integer_intervalt;
typedef intervalt<ieee_floatt> ieee_float_intervalt;
